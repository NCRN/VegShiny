...
session$onFlushed(once=TRUE, function() {   ##onFlushed comes superzip - makes map draw befrore circles
 # paintObs <- observe({
  ...
  map$addCircle(getPlots(NCRN)$Latitude,getPlots(NCRN)$Longitude, 10*PlantVals(), options=list(color= brewer.pal(7, "Spectral")[cut(PlantVals(), 7, labels = FALSE)],fillOpacity=.75))

...
TARGET_COLUMNS <- c("latin_name", "tsn", "plot_name", "l_Unit_Code") # naming-convention: constants (i.e., hard-coded variables that don't change) should be upper case

is_preprocessing_necessary <- function(Network, target_columns) {
    ...
    preprocess_val <- any(sapply(preprocess, function(x,folderpath) length(x) > 0))
    
    return(preprocess_val)
}

needs_preprocessing_bool <- is_preprocessing_necessary(Network=Network, target_columns=TARGET_COLUMNS)
